...
>
> Это форк [akalikbergenov/cyclop](https://github.com/akalikbergenov/cyclop) —
> оригинальное приложение написал [@akalikbergenov](https://github.com/akalikbergenov),
> ему же принадлежит вся заслуга за исходную идею, челку-панель, плеер, календарь
> и перевод. Здесь к этому добавлены буфер обмена под клавиатуру, снимки экрана с
> редактором и распознаванием текста, настройки и ещё несколько вещей —
> [полный список ниже](#что-добавлено-в-этой-версии).
>
> Апстрим: <https://github.com/akalikbergenov/cyclop> ·
> Лицензия та же, MIT.

Челка MacBook как рабочий инструмент. Нативное приложение на SwiftUI/AppKit: в покое
невидимо, разворачивается вниз в панель с плеером, общим буфером для копий и файлов,
снимками экрана, заметками и ближайшими встречами.

![Панель Cyclop](docs/panel.png)

```
0.0 % CPU в покое  ·  ≈40 МБ + 14 МБ helper  ·  ~2 МБ бандл  ·  разрешения только по кнопке
```

## Установка

**[⬇ Скачать последнюю версию](https://github.com/marmeladov98/cyclop/releases/latest)** ·
macOS 15 или новее · ~1,2 МБ

1. Открыть скачанный `Cyclop-<версия>.dmg` и перетащить **Cyclop** в «Программы».
2. Запустить. Первый раз macOS откажет: образ подписан ad-hoc, без Developer ID.
   Идти в **Системные настройки → Конфиденциальность и безопасность**, внизу
   будет строка про Cyclop и кнопка **«Все равно открыть»**.
   Либо одной командой в Терминале:
   ```bash
   xattr -dr com.apple.quarantine /Applications/Cyclop.app
   ```
3. Всё. Иконка-глаз появится в меню-баре, панель — по наведению на челку.

Два разрешения приложение спросит само и только когда понадобится:
**Универсальный доступ** — чтобы вставлять выбранное из буфера по ⌘⌥V,
**Запись экрана** — чтобы делать снимки по ⌘⇧A. Обоими можно не пользоваться, и
тогда их не спросят. После выдачи разрешения приложение нужно один раз
перезапустить.

Обновление — тем же способом: открыть новый образ и заменить приложение.
Версия видна в меню-баре первой строкой.

> **Нет готового образа в релизах?** Значит собери сам — это одна команда:
> ```bash
> git clone https://github.com/marmeladov98/cyclop && cd cyclop && ./Scripts/dmg.sh
> ```
> Нужны macOS 15 и Command Line Tools (`xcode-select --install`). Готовый
> `build/Cyclop-<версия>.dmg` открывается так же, как выше.

## Что добавлено в этой версии

Всё перечисленное — надстройка над оригиналом; всё, что не названо, работает как
у автора.

**Буфер обмена под клавиатуру — ⌘⌥V.** Список последних копий открывается
сочетанием клавиш посередине экрана: стрелки выбирают, Return вставляет запись
прямо туда, где стоит каретка. Есть ⌘1…⌘9, ⌫ для удаления записи, esc, повторное
нажатие ⌘⌥V идёт вниз по списку. Автовставка требует Универсального доступа —
если его нет, запись всё равно оказывается в буфере обмена.

**«Полка» и «Буфер» объединены в один список.** Раньше скопированный текст жил в
одной вкладке, а файлы и картинки — в другой. Теперь это одна вкладка «Буфер»:
текст, ссылки, файлы, картинки, видео, с превью от QuickLook, перетаскиванием
файлов наружу и ⌘-кликом для группы. Записи со старой полки переносятся сами.

**Снимки экрана с редактором — ⌘⇧A.** Экран замирает, выделяется область (или
окно — одним кликом, со скруглением углов; или весь экран — кликом по меню-бару),
поверх можно рисовать: прямоугольник, овал, стрелка, линия, карандаш, маркер,
текст, пикселизация. Нарисованное остаётся объектом: его можно двигать,
растягивать за ручки, перекрашивать, отменять. ⌘C копирует, ⌘S сохраняет в
`~/Pictures/Cyclop`.

**Распознавание текста и перевод.** Кнопка в панели снимка читает текст с
выделенной области через Vision — на устройстве, без сети. Результат открывается
отдельным окном: слева миниатюра, справа текст с выбором языка распознавания,
копированием и переводом через тот же офлайновый переводчик, что и вкладка
«Перевод».

**Вкладка настроек.** Размер буфера, хранение текста между запусками, сохранение
картинок, три горячих сочетания с записью нажатием, размеры панели и челки,
автозапуск, «скрывать содержимое», раскрытие панели наведением или кликом.

**Исправления в исходном коде.** История копирований больше не пропадала при
подключении монитора — хранилище жило внутри панели, а панель пересобирается при
любой смене конфигурации экранов.

**Сборка.** `Scripts/bundle.sh` подписывает устойчивой личностью, если она есть
(`CODESIGN_IDENTITY`, Developer ID или самоподписанный сертификат с именем
«Cyclop»), и объясняет, чем грозит ad-hoc: TCC привязывает выданные разрешения к
хешу бинарника, и они слетают при каждой пересборке.

Подробности по каждой версии — в [`docs/releases`](docs/releases).
...
| **Буфер** | Всё скопированное и перетащенное — одним списком: текст, ссылки, файлы, картинки. Клик возвращает запись в буфер обмена. Файлы тянутся наружу прямо в Finder — ⌘-клик выбирает несколько, и уходит вся группа. Скопированная картинка сохраняется файлом и остаётся, в том числе снятая на айфоне. **⌘⌥V** открывает тот же список под клавиатуру: стрелки выбирают, Return вставляет туда, где ты печатал |
...
| **Снимок экрана** | **⌘⇧A** замораживает экран, ты выделяешь область и рисуешь на ней — прямоугольники, овалы, стрелки, линии, карандаш, маркер, текст, пикселизация — и всё нарисованное потом можно взять и подвинуть или растянуть. ⏎ копирует, буфер подхватывает как любую скопированную картинку. Вторая половина — чтение: наводишь на область, и текст с картинки становится текстом, который можно вставить, на любом языке, который знает Vision, и тут же переводится |
| **Настройки** | Сколько записей хранит буфер, переживает ли текст перезапуск, оба сочетания клавиш, размеры панели и челки, в которую она сворачивается |
...
Во время перетаскивания файлов она раскрывается сама и сразу показывает буфер.
Иконка в меню-баре — переключить панель, включить автозапуск, выйти.

**⌘⌥V — буфер без курсора.** Список появляется посередине экрана, последняя
копия уже выбрана, и фокус у приложения, в котором ты печатал, никто не
забирает.

| Клавиша | |
|---|---|
| `↑` `↓` | ходят по истории, с обоих концов по кругу |
| `⇥` / `⇧⇥` | то же самое — для руки, которая уже там |
| `⇞` `⇟` `↖` `↘` | на экран за раз, либо сразу в начало и в конец |
| `⌘1`…`⌘9` | сразу на одну из первых девяти записей |
| `⌥⌘V` ещё раз | на строку ниже, как ⌘⇥ ходит по переключателю приложений |
| `⏎` | положить в буфер и вставить туда, где стоит каретка: текст текстом, картинку картинкой, файл файлом |
| `⌫` | выбросить запись из истории, не закрывая список |
| `esc` | закрыть, ничего не меняя |

**⌘⇧A — снимок экрана.** По нажатию экран замирает, поэтому выделяется тот
кадр, на который ты смотрел, а не тот, до которого всё успело доехать, пока ты
тянул рамку. За курсором едет лупа с размером и цветом под перекрестием; `C`
копирует этот цвет.

| Клавиша | |
|---|---|
| навести на окно | оно обводится синим пунктиром; один клик берёт его целиком, вместе со скруглением углов |
| навести на меню-бар | предлагается весь экран целиком |
| тянуть | выделить область; ручки её перекроят, а нажатие вне рамки начнёт новую |
| `⌘C` / `⏎` | скопировать снимок и закрыть |
| `⌘S` | сохранить в `~/Pictures/Cyclop`, не трогая буфер обмена |
| `⌫` | удалить выбранную аннотацию |
| `⌘Z` / `⇧⌘Z` | отменить, вернуть |
| указатель, нажать внутри | перетаскивает саму рамку; нажатие на нарисованное двигает его |
| `esc` | выйти из набора текста; второе нажатие закрывает снимок |
| `C` | скопировать цвет под перекрестием, пока выделяешь |

Инструмент **«Перемещать»** — то, ради чего рисование вообще имеет смысл:
стрелку, улетевшую не туда, можно поймать за древко и перетащить, рамку — за
край, а ручки растягивают выбранное: восемь у рамки, две у стрелки — хвост и
остриё. Выбранное слушается и палитры с ползунком толщины, так что перекрасить
только что нарисованное можно, не удаляя его. Фигура, которую ты дорисовал
прямо сейчас, уже выбрана и двигается без переключения инструмента. Ничего не
впечатывается в картинку, пока снимок не скопирован или не сохранён.

**Распознавание текста** — вторая половина. Выделяешь область, жмёшь кнопку с
текстом, и то, что было пикселями, возвращается текстом: Vision, на устройстве,
ничего никуда не уходит. Прочитанное открывается отдельным окном: слева
снятый кусок, справа текст, меню языка, кнопка копирования и перевод со своей.
«Автоматически» идёт за твоими языками и чаще всего право; явно названный язык
спасает случай, когда нет, — кириллица, прочитанная как латиница, возвращается
уверенной бессмыслицей, а не ошибкой, — и смена языка перечитывает ту же
картинку. Перевод идёт через тот же офлайновый переводчик, что и вкладка
«Перевод».

Все три сочетания меняются в настройках — там же есть второе, которое открывает
саму панель; по умолчанию оно не назначено.

Вставить за тебя — значит нажать ⌘V за тебя, а этого macOS не даёт делать без
спроса, см. [Разрешения](#разрешения). Пока галочка не стоит, сочетание делает
всё остальное: запись оказывается в буфере, а последний ⌘V за тобой.
...
Готовые образы лежат на [странице релизов](https://github.com/marmeladov98/cyclop/releases) —
это и есть ссылка, которую можно давать людям вместо файла.

## Разрешения

**Ни одного** — пока не откроешь календарь, и ни одного самой панели.
Приложение не просит ни Автоматизацию, ни Запись экрана, и ничего не требует
...
Само сочетание ⌘⌥V регистрируется в оконном сервере: он сообщает приложению о
нажатии именно этой комбинации и ничего больше о том, что набирают, — поэтому
оно работает сразу после установки, отмечать ничего не надо.
...
**Запись экрана** нужна инструменту снимков, и по очевидной причине: снять
экран, не имея права его читать, нельзя. Спрашивается при первом нажатии ⌘⇧A, не
при запуске. Не делаешь снимков — приложение и не спросит.

**Универсальный доступ** нужен ровно одному месту — автоматической вставке в
конце ⌘⌥V. Нажать ⌘V в чужом приложении значит синтезировать ввод, а это macOS
разрешает только по галочке. Диалог появляется при первом выборе записи, а не
при запуске, и отказ стоит ровно одного нажатия: запись в буфере в любом случае,
дальше ⌘V руками. Право выдаётся приложению на диске, поэтому после галочки
macOS требует один раз перезапустить Cyclop. Пока разрешения нет, в меню-баре
висит пункт «Разрешить автовставку…».

> **Галочка стоит, а оно всё равно спрашивает?** Это ad-hoc подпись, а не
> сломанное разрешение. У ad-hoc подписи нет устойчивого designated requirement,
> поэтому macOS привязывает выданное право к хешу конкретного бинарника — а он
> меняется при каждой пересборке. Строчка, оставшаяся в списке, относится к
> приложению, которого там больше нет. Удали Cyclop из «Конфиденциальность и
> безопасность → Универсальный доступ» кнопкой «−», перезапусти приложение и
> разреши заново. Чтобы это не повторялось на каждой сборке, нужна устойчивая
> личность подписи: `Scripts/bundle.sh` сам подхватит Developer ID, любой
> самоподписанный сертификат для подписи кода с именем «Cyclop» (Связка ключей →
> Ассистент сертификации → Создать сертификат) или то, что назовёт
> `CODESIGN_IDENTITY`.
...
копию до мака, и снимок оказывается в буфере. Ни одного тапа, ни облака, ни общей
...
свежее намерение, и дотягивать замещённую передачу значило бы положить в буфер
...
- Буфер ссылается на файлы, а не копирует их: если переместить оригинал, запись
  ...
  даже когда запись уходит из буфера. Чистит папку только пользователь: пункт
...
│   ├── BufferPickerPanel.swift       окно ⌘⌥V и его коды клавиш
│   └── BufferPickerController.swift  список: горячая клавиша, клавиши, вставка
├── Model/
│   ├── NotchViewModel.swift
│   └── ShotDocument.swift     аннотации: попадания, перемещение, отмена
├── Shot/
│   ├── ShotController.swift   сочетание, оверлеи, во что превращается снимок
│   ├── ShotPanel.swift        полноэкранный оверлей и лупа
│   └── ShotCanvasView.swift   выделение, рисование, перетаскивание
├── Services/
...
│   ├── BufferStore.swift      одна история: копии, файлы и картинки
│   ├── ScreenCapture.swift    замороженный кадр под редактором снимка
│   ├── TextRecognizer.swift   Vision: пиксели обратно в текст
│   ├── Settings.swift         что меняет пользователь и кто за этим следит
│   ├── HotKey.swift           глобальные сочетания, без разрешений
│   ├── Paster.swift           синтезированный ⌘V и право его отправить
