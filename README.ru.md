> [!WARNING]
> **Это бета-ветка.** Здесь лежит то, про что ещё не решено, место ли ему в
> Cyclop. Готового образа у беты нет, обновлений тоже — собирается из
> исходников и ломается без предупреждения.
>
> Стабильная версия — в [`main`](https://github.com/akalikbergenov/cyclop/tree/main)
> и в [релизах](https://github.com/akalikbergenov/cyclop/releases/latest).
> Правила канала — в [docs/beta.md](docs/beta.md).

## Что здесь пробуют

| Фишка | Что делает | Обсуждение |
|---|---|---|
| **Составная поверхность** | музыка, ближайшая встреча и последние копирования на одном экране; рельс с десяти икон сокращён до пяти | [#100](https://github.com/akalikbergenov/cyclop/issues/100) |
| **Живая чёлка** | панель опускается на строку, когда воткнули шнур или сам сменился трек | — |
| **Визуализатор** | настоящий спектр играющего звука. **Выключен по умолчанию**: включается в «Настройки → Визуализатор», и при включении macOS спросит доступ к записи системного звука | [#103](https://github.com/akalikbergenov/cyclop/issues/103) |
| **Закреплённая плашка** | спектр и что играет остаются под вырезом, когда панель свёрнута | [#103](https://github.com/akalikbergenov/cyclop/issues/103) |

Визуализатор — единственное место, где бета отступает от обещания «ноль
разрешений», и именно поэтому он выключен, пока не решено, оставлять ли его
вообще.

### Дом

![Дом: музыка, встреча и буфер на одной поверхности](docs/beta/home-surface.png)

Слева музыка с настоящим спектром играющего, справа ближайшая встреча с
отсчётом и последние копирования. Раньше это были три вкладки, и посмотреть на
них разом было нельзя: открыт календарь — не видно, что играет.

### Инструменты

![Инструменты: перевод, валюта и суфлёр под одной иконкой](docs/beta/tools.png)

Перевод, конвертер и суфлёр занимали три иконки из десяти, а открываются реже
всего остального. Теперь это одна иконка и переключатель внутри: рельс — про
то, куда идти, а не про то, в каком ты режиме.

Всего на рельсе осталось пять икон вместо десяти.

### Закреплённая плашка

![Плашка под вырезом: обложка, трек и спектр](docs/beta/pinned-bar.png)

Спектр видно, только пока панель раскрыта наведением. Для всего остального в
панели это правильно, но не для музыки: её слушают часами, а панель открывают
на секунды. Закрепление оставляет обложку, название и спектр под вырезом.

Плашка не мешает: наведение на вырез открывает панель поверх неё, а нажатие
мимо выреза уходит в меню-бар. Кнопок в ней нет — открепить можно с домашней
поверхности и из настроек.

### Где включить спектр

![Настройки: выключатель визуализатора](docs/beta/settings.png)

Первым же пунктом в настройках. При включении macOS спросит доступ к записи
системного звука — это и есть то самое третье разрешение, из-за которого судьба
фишки не решена.
...
Вернуться на стабильную — скачать образ из релизов и заменить приложение.
Настройки лежат в `~/Library/Application Support/Cyclop` и переживают замену.

> [!NOTE]
> **После каждой пересборки разрешения слетают.** `Scripts/bundle.sh` подписывает
> бандл заново, а macOS привязывает выданные разрешения к подписи — для системы
> это каждый раз новое приложение. Доступ к Календарю и к записи звука придётся
> выдавать снова. Это не поломка беты, а следствие сборки из исходников без
> сертификата Developer ID.
...
**Зона в свернутом виде.** Настоящая челка — дыра, и панель забирает ее целиком:
под ней нечему перехватывать клик. Синтетическая нарисована поверх работающего
меню-бара, и там, где до нее дотягиваются иконки, зона сжимается до полоски у
самой верхней кромки: до нее по-прежнему добираешься броском курсора вверх, а
курсор, идущий к иконке, проходит ниже.

Какой из двух случаев на экране — измеряется для каждого дисплея, а не выводится
из модели мака. Статус-итемы это окна на уровне статус-бара, и рамка окна
публична, даже когда его картинка нет, — поэтому самый левый из них просто
считывается: на одном 13-дюймовом маке они начинаются с x≈757, на другом с
x≈1158, а на внешнем экране шириной 1920 pt челка отстоит от них на добрых
550 pt. Челка, до которой иконки не дотягиваются, ведет себя как настоящая:
полная глубина и та же задержка в 50 мс вместо 300. Замер повторяется каждый
раз, когда панель сворачивается, — иконки появляются и исчезают вместе с
приложениями, которым принадлежат.
