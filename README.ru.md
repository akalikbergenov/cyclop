...
| **Посты** | Скопируй ссылку на пост в X — «Скопировать ссылку» на самом посте и есть весь жест — и он сохранён на потом: автор, когда сохранено, заметка, если добавить. Клик открывает пост в браузере; счётчик в шапке считает непрочитанные; поиск ищет по авторам и заметкам. В сеть ничего не ходит — всё, что видно в строке, разобрано из самой ссылки. Список лежит в `~/Library/Application Support/Cyclop/posts.json`; вкладка выключается в настройках, и сбор останавливается вместе с ней |
