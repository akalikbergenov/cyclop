...
final class KeyboardLock: ObservableObject {
    ...
    func requestAccess() {
        // The key by name rather than through `kAXTrustedCheckOptionPrompt`:
        // that global is a `var` in the Carbon headers and strict concurrency
        // refuses to touch it. Its value is fixed API.
    ...
    func lock() -> Refusal? {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            callback: { _, type, event, context in
                ...
                // The event itself never leaves this callback: `CGEvent` is not
                // `Sendable`, and the decision needs only its type and key code.
                let code = event.getIntegerValueField(.keyboardEventKeycode)
                let verdict = MainActor.assumeIsolated { lock.handle(type, code) }
                return verdict ? nil : Unmanaged.passUnretained(event)
            },
    ...
    /// Returns whether the event is to be swallowed.
    private func handle(_ type: CGEventType, _ code: Int64) -> Bool {
